(ns motion.demo-curved-path
  (:require [reagent.core :as reagent :refer [atom]]
            ...
            [motion.utils :refer [g-trans hexagon]]
...
(defn component-svg-rounded-hexagon [style x y r]
  [:g (merge style (g-trans x y))
   [:path {:d (js/roundPathCorners (str "M " (string/join " L " (hexagon r false)) " Z") 5 false)}]])

(defn component-svg-hexagon [style x y r]
  [:g (merge style (g-trans x y))
   [:path {:d (str "M " (string/join " L " (hexagon r false)) " Z")}]])
  ...
  (let [over (atom false)]
    (fn []
      [:g (merge style {:on-mouse-over (fn [ev] (reset! over true) nil) :on-mouse-out (fn [ev] (reset! over false) nil)})
       ...
       (if @over
         [:g
          (components/component-svg-arc style x y 78 (* m.PI 0.75) (* m.PI 1.25) 2)
          (components/component-svg-arc style x y 78 (* m.PI -0.25) (* m.PI 0.25) 2)
          (components/component-svg-arc style x y 70 (* m.PI 0.75) (* m.PI 1.25) 5)
          (components/component-svg-arc style x y 70 (* m.PI -0.25) (* m.PI 0.25) 5)])])))
