    ...
    (fn []
      (let [t (/ @tl 3000)
        ...
        [:g
         ...
         [:g {:transform (str "translate(" (* (.cos m t) distance) "," (* (.sin m t) distance) ")")}
          [:circle {:cx 0 :cy 0 :r 30 :fill (colors :background) :stroke-width "2px" :on-click (partial handle-orbiter-click unfolder)}]
          (if (= unfolded 0)
            [:circle {:cx (+ (* (.cos m t2) 80)) :cy (+ (* (.sin m t2) 80)) :r 10 :fill (colors :background)}])
          ; unfolder
          (if (> unfolded 1800)
            [:path {:d (str "M 21 -21 L 70 -70" (if (> unfolded 2000) "L 200 -70" "")) :stroke-width "2px"}])
          (when (> unfolded 0)
            [:g (if (> unfolded 900) {:transform (str "rotate(" (.min m 90 (/ (- unfolded 900) 10)) ")")})
             (components/component-svg-arc style-alert 0 0 70 (* m.PI 0.75) (* m.PI 1.25) 5)
             (components/component-svg-arc style-alert 0 0 70 (* m.PI -0.25) (* m.PI 0.25) 5)])
          (when (> unfolded 300)
            [:g
             (components/component-svg-arc style-alert 0 0 78 (* m.PI 0.75) (* m.PI 1.25) 2)
             (components/component-svg-arc style-alert 0 0 78 (* m.PI -0.25) (* m.PI 0.25) 2)])
          (when (> unfolded 600)
            [:g
             (components/component-svg-arc style-alert 0 0 85 (* m.PI 0.75) (* m.PI 1.25) 2)
             (components/component-svg-arc style-alert 0 0 85 (* m.PI -0.25) (* m.PI 0.25) 2)])
